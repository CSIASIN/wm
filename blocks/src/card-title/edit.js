...
export default function Edit( { attributes, setAttributes } ) {
    const { 
        level, content, displayClass, textAlign, textColor, fontWeight, customClass,
    ...
    const headingClass = [
        displayClass, textAlign, textColor, fontWeight, customClass
    ].filter( Boolean ).join( ' ' );

    const blockClasses = [
        'wmblocks-bs-heading',
        hasVideo ? 'position-relative overflow-hidden' : '',
        headingClass, padding, margin, backgroundColor, borderClasses, visibilityClasses, shadow
    ].filter( Boolean ).join( ' ' );
    ...
    return (
        <>
            ...
            <Tag { ...blockProps }>
                { /* Editor Administrative Overlay Badges */ }
