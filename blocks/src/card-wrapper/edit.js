...
export default function Edit( { attributes, setAttributes } ) {
    ...
    const CARD_TEMPLATE = [
        [ 
            'core/paragraph', 
            { 
                placeholder: __( "Add your container content here... This card component acts like a container box to style, bundle, and organize your layouts together inside a single cleanly visible component wrapper element structure.", 'wm' ),
            } 
        ]
    ];
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={ __( 'Card Settings', 'wm' ) } initialOpen={ true }>
                    <TextControl
                        label={ __( 'Card Width', 'wm' ) }
                        help={ __( 'Set explicit maximum bounds width limits (e.g., 18rem, 350px, 100%).', 'wm' ) }
                        value={ cardWidth }
            ...
            <div { ...blockProps }>
                { hasVideo ? (
                    ...
                    <InnerBlocks template={ CARD_TEMPLATE } renderAppender={ InnerBlocks.ButtonBlockAppender } />
                ) }
            </div>
        </>
    );
