...
export default function Edit( { attributes, setAttributes } ) {
	const { iconSvg, iconColor, heading, subtext, btn1Text, btn1Url, btn1Variant, btn2Text, btn2Url, btn2Variant, bgColor, colWidth, customClass } = attributes;
		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Icon', 'wmblocks' ) } initialOpen={ true }>
					<SelectControl label={ __( 'Icon Color', 'wmblocks' ) } value={ iconColor } options={ ICON_COLOR_OPTS } onChange={ v => setAttributes( { iconColor: v } ) } />
					<TextareaControl label={ __( 'Custom SVG', 'wmblocks' ) } value={ iconSvg } onChange={ v => setAttributes( { iconSvg: v } ) } rows={ 4 }
						help={ __( 'Paste any <svg> markup. The default is a question-mark circle icon.', 'wmblocks' ) } />
				</PanelBody>
				...
				<PanelBody title={ __( 'Style', 'wmblocks' ) } initialOpen={ false }>
					...
					<SelectControl label={ __( 'Text Column Width', 'wmblocks' ) } value={ colWidth } options={ COL_OPTS } onChange={ v => setAttributes( { colWidth: v } ) }
						help={ __( 'Controls the responsive width of heading and text to control line length.', 'wmblocks' ) } />
