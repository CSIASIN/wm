...
export default function AnimationControls({ attributes, setAttributes }) {
    ...
    return (
        <InspectorControls>
            {/* Dedicated Animations Panel, closed by default to save space */}
            <PanelBody title={ __( 'Animations', 'wmblocks' ) } initialOpen={ false }>
                ...
                {wmAnim && wmAnim !== 'none' && (
                    <div style={{ marginBottom: '24px', paddingLeft: '12px', borderLeft: '2px solid #ccc' }}>
                        ...
                        />
                        <TextControl 
                            label={ __( 'Duration (ms)', 'wmblocks' ) } 
                            value={wmDuration || '400'} 
                            onChange={(val) => setAttributes({ wmDuration: val })} 
                        />
                        <TextControl 
                            label={ __( 'Delay (ms)', 'wmblocks' ) } 
                            value={wmDelay || '0'} 
                            onChange={(val) => setAttributes({ wmDelay: val })} 
                        />
                        <PanelRow>
                            <ToggleControl 
                                label={ __( 'Animate Once', 'wmblocks' ) } 
                                checked={wmOnce !== false} 
                                onChange={(val) => setAttributes({ wmOnce: val })} 
                            />
                        </PanelRow>
                        <PanelRow>
                            <ToggleControl 
                                label={ __( 'Mirror (Animate Out)', 'wmblocks' ) } 
                                checked={!!wmMirror} 
                                onChange={(val) => setAttributes({ wmMirror: val })} 
                            />
                        </PanelRow>
                    </div>
                )}

                {/* --- FUTURE: ON HOVER ANIMATIONS --- */}
                {/* <SelectControl
                    label={ __( 'Hover Animation', 'wmblocks' ) }
                    ...
                /> 
                */}

                {/* --- FUTURE: CONTINUOUS ANIMATIONS --- */}
                {/* <SelectControl
                    label={ __( 'Continuous Animation', 'wmblocks' ) }
                    ...
                /> 
                */}
